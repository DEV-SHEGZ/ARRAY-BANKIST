...
and the LAST TWO dogs actually have cats, 
...
log to the console whether it's an adult 
...
const checkDogs = function (dogsJulia, dogsKate) {
  const juliaChanged = dogsJulia.slice(1, -2);
  const dogsCombined = juliaChanged.concat(dogsKate);

  dogsCombined.forEach(function (Age, i) {
    const Adult = Age < 3 ? "Puppy" : "Adult";
...
const Owner1 = [5, 2, 4, 1, 15, 8, 3];
const owner2 = [16, 6, 10, 5, 6, 1, 4];

console.log(`--NEW ASSESSMENT--`);

const calcAverageHumanAge = function (ages) {
  let humanAge = 0;
  const newAge = ages.map(function (age) {
    if (age <= 2) {
  ...
  const onlyDogs = newAge.filter((age) => age > 18);

  const totalAvgAdult = onlyDogs.reduce(function (acc, val, i, arr) {
    return acc + val;
  }, 0);

  return totalAvgAdult / onlyDogs.length;
};

console.log(calcAverageHumanAge(Owner1));
console.log(calcAverageHumanAge(owner2));

const calcalcAverageHumanAgeArr = (ages) => {
  const newAge1 = ages
...
console.log(calcalcAverageHumanAgeArr(owner2));
console.log(calcalcAverageHumanAgeArr(Owner1));
